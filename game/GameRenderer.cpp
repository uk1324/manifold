...
GameRenderer GameRenderer::make() {
	...
	GameRenderer renderer{
		...
		.coloredTriangles = TriangleRenderer<Vertex3Pn>::make<ColoredShader>(instancesVbo),
		...
		#else
		//.font = Font::loadSdfWithCachingAtDefaultPath(FONT_FOLDER, "RobotoMono-Regular", "ttf", NUMBERS_CHARACTER_RANGES),
		.font = loadFontSdfFromMemory(fontHeight, fontGlyphs(), fontImage, fontImageSizeX, fontImageSizeY),
		#endif
	...
	t.tookSeconds("initializing GameRenderer");
...
void GameRenderer::renderColoredTriangles(const ColoredInstance& instance) {
	shaderSetUniforms(coloredShader, ColoredShadingVertUniforms{
		.transform = transform,
		.view = view,
	});
	//.model = coloredShadingModel,
	renderTriangles(coloredShader, coloredTriangles, instancesVbo, instance);
}

void GameRenderer::coloredTrianglesAddMesh(const std::vector<Vec3>& positions, const std::vector<Vec3>& normals, const std::vector<i32>& indices, Vec3 color) {
	const auto offset = i32(coloredTriangles.vertices.size());
	for (i32 i = 0; i < positions.size(); i++) {
		coloredTriangles.addVertex(Vertex3Pn{
	...
	for (const auto& index : indices) {
		coloredTriangles.indices.push_back(offset + index);
	}
}

void GameRenderer::coloredTrianglesAddMesh(const LineGenerator& lineGenerator, Vec3 color) {
	coloredTrianglesAddMesh(lineGenerator.positions, lineGenerator.normals, lineGenerator.indices, color);
}
